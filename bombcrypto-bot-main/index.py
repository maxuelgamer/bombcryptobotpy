...
def scroll():
    ...
    if (len(commoms) == 0):
        print('Nenhum Comum encontrado')
        return
    x,y,w,h = commoms[len(commoms)-1]
    print('Movendo para {},{}'.format(x,y))
...
def clickButtons(trashhold=.95):
    ...
    print('Botões: {}'.format(len(buttons)))
    for (x, y, w, h) in buttons:
        pyautogui.click(x+(w/2),y+(h/2))
        global hero_clicks
        hero_clicks = hero_clicks + 1
        print('{} Heróis mandados até agora'.format(hero_clicks))
...
def login():
    ...
    if clickBtn(signBtn):
        ...
        if clickBtn(teasureHunt):
            print('Logado com sucesso, Botão do mapa clicado')
            login_attempts = 0
        time.sleep(15)
        return
        # click ok button
...
def main():
    ...
    while True:
        ...
        if now - last["heroes"] > 60 * 30:
            last["heroes"] = now
            print('Mandando os escravos pra trabalhar')
            refreshHeroes()
